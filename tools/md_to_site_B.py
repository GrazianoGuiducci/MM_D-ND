#!/usr/bin/env python3
"""
md_to_site_B.py — Convert Paper B draft to site-ready format for d-nd.com
...
def main():
    print("=" * 60)
    print("Paper B → Site-Ready Conversion")
    print("=" * 60)

    # Read source
    source_file = INPUT_DIR / "paper_B_draft3.md"
    ...
    # Save metadata
    meta_file = OUTPUT_DIR / "paper_B_metadata.json"
    ...
    site_file = OUTPUT_DIR / "paper_B.md"
    ...
    print(f"Files:")
    for f in sorted(OUTPUT_DIR.rglob("paper_B*")):
        if f.is_file():
            size = f.stat().st_size
