...
5. **Lindblad master equation for emergence dynamics with quantitative decoherence rate** $\Gamma = \sigma^2/\hbar^2 \cdot \langle(\Delta\hat{V}_0)^2\rangle$, explaining non-unitary dynamics through intrinsic potential fluctuations.
...
This conservation law — the *complementarity of potential and actuality* — is the informational analogue of energy conservation. The emergence measure $M(t)$ tracks the fraction of modes that have transitioned from $\mathcal{P}$ to $\mathcal{A}$: $M(t) = |\mathcal{A}(t)|/\dim(\mathcal{H})$ in the discrete case. The arrow of emergence (§3.5) is the statement that this transfer is statistically irreversible under the conditions of Theorems 1–2.
...
where $\Gamma_n = (\sigma^2/\hbar^2)|\langle n|\hat{V}_0|m\rangle - \langle m|\hat{V}_0|m\rangle|^2$ are the state-dependent decoherence rates. This provides *exponential* convergence to emergence, in contrast to the oscillatory convergence of the purely unitary case (Proposition 1).

**Remark (Status of Decoherence Rate):** The form $\Gamma = \sigma^2/\hbar^2 \cdot \langle(\Delta\hat{V}_0)^2\rangle$ is a phenomenological ansatz motivated by dimensional analysis and consistency with the Fermi Golden Rule in the weak-coupling limit. Specifically: (1) $\sigma^2/\hbar^2$ provides the correct dimensions of $[\text{time}]^{-1}$; (2) $\langle(\Delta\hat{V}_0)^2\rangle$ measures the variance of the pre-differentiation landscape, which physically controls the transition rate between emergence sectors; (3) in the limit of Gaussian-distributed $V_0$ fluctuations, this form reduces to the standard Caldeira-Leggett result for quantum Brownian motion (Caldeira & Leggett 1983). A rigorous derivation from the Lindblad master equation, starting from the D-ND Hamiltonian decomposition (§2.5), remains an open problem.
...
**Remark (Analytic Continuation):** The contour integral above requires extending $Z(t) \in [0,1]$ (real) to the complex $Z$-plane. This analytic continuation is justified by the Schwarz reflection principle: the effective potential $V_{\text{eff}}(Z)$ is a polynomial in $Z$, hence entire, and its analytic continuation to $\mathbb{C}$ is unique. The contour $C$ encircles the two turning points $Z = 0$ and $Z = 1$ in the complex plane, where $V_{\text{eff}}$ has simple poles in the integrand $1/\sqrt{E - V_{\text{eff}}}$. The winding number is +1 for both poles (standard for a contour encircling two simple poles with positive orientation), yielding $\text{Res}_{Z=0} + \text{Res}_{Z=1} = i + i = 2\pi i$ after normalization by the $2\pi$ from the residue theorem. This is a standard WKB-type quantization condition (analogous to the Bohr-Sommerfeld rule $\oint p \, dq = 2\pi n\hbar$); the imaginary unit reflects the tunneling character of the orbit connecting the two potential minima.
...
(iv) **The Lindblad dynamics** (with $\sigma/\hbar = 0.1\omega_0$) show exponential convergence as predicted by §3.6, with rate matching $\Gamma$ within $3\%$.
